...
gs4_auth(email = "sunnyrafter@gmail.com")
...
ui <- fluidPage( titlePanel("Interactive Exercise"),
                 sidebarLayout( sidebarPanel( textInput("name", "Enter your name:"),
                                              textAreaInput("code", "Write your R code here:", "", rows = 5),
                                              actionButton("submit", "Submit") ),
                                mainPanel( textOutput("confirmation") ) ) )

server <- function(input, output, session) {
  observeEvent(input$submit,
               {

                 response <- data.frame( Name = input$name, Code = input$code, Timestamp = Sys.time() )
                 sheet_append(sheet_id, response, sheet = sheet_name)


                 output$confirmation <- renderText({ paste("Thank you,", input$name, "! Your response has been recorded.") })
               }) }
